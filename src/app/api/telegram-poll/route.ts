...
// Глобальная переменная для хранения offset
let lastUpdateId = 0;
...
async function handleUpdate(update: { message?: { chat: { id: number }; text?: string; contact?: { first_name: string; last_name?: string; phone_number: string; user_id: number } } }) {
  ...
  console.log('Processing message:', { chatId, text, hasContact: !!contact });
...
💬 Оставлять комментарии мастеру
  ...
  if (contact) {
    ...
    console.log('Registering user:', { name, phone, telegramId });
...
async function getUpdates() {
  ...
  try {
    ...
    if (data.ok && data.result && data.result.length > 0) {
      ...
      return { processed: data.result.length };
    }
...
export async function GET(request: NextRequest) {
  // Poll for updates when this endpoint is called
  const result = await getUpdates();
  return NextResponse.json({ 
    status: 'Telegram polling executed',
    ...result,
    lastUpdateId 
  });
}
...
export async function POST(request: NextRequest) {
  // Manual trigger for processing updates
  const result = await getUpdates();
  return NextResponse.json({ 
    status: 'Telegram polling executed',
    ...result,
